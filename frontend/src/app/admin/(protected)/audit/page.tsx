        ...
        {logs.length === 0 ? (
          ...
          <>
            <div className="grid gap-4 md:hidden">
              {logs.map((log) => (
                <MobileRecordCard key={log.id} title={log.actor_email}>
                  <MobileRecordRow label="Action" value={log.action} />
                  <MobileRecordRow label="Entity" value={log.entity_type} />
                  <MobileRecordRow label="Date" value={formatDate(log.created_at)} />
                </MobileRecordCard>
              ))}
            </div>
